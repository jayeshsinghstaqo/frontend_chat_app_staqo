'use client'
import React, { useState } from 'react';
import { useRouter } from 'next/navigation';
import { Button, TextField, Grid, Container, CircularProgress } from '@mui/material';
import axios from 'axios';
import toast from "react-hot-toast";
import { useAuthContext } from '@/contexts/AuthContext';
import { base_url } from '@/helpers/api/api';
// import Link from 'next/link';

const Login = () => {
    const router = useRouter()
    const [otpShow, setOtpShow] = useState(false);
    const [error, setError] = useState(false);
    const [loading, setLoading] = useState(false);
    const [data, setData] = useState({
        mobile_number: '',
        otp: '',
        error: ''
    });
    const { authUser, setAuthUser } = useAuthContext();

    const handleChange = (e) => {
        const { name, value } = e.target;
        setData({
            ...data,
            [name]: value
        });
    };

    const loginFun = async (e) => {

        e.preventDefault();
        if (otpShow) {
            try {
                let res = await axios.post(`${base_url}/api/auth/verify-otp`, {
                    mobile_number: data.mobile_number,
                    otp: data.otp
                })
                console.log(res, 'this is response of validate otp')

                if (res.status === 200) {
                    if (is_profile_complete == false) {
                        router.push('/auth/signup')
                    } else {
                        router.push('/')
                    }
                    setData({
                        ...data,
                        error: ""
                    })
                }
                

            } catch (error) {
                setLoading(false)
                if (error) {
                    setData({
                        ...data,
                        error: error.response.data.message
                    })
                }
            }
        }
        else {
            if (!data.mobile_number || data.mobile_number.length !== 10) {
                setError(true);
                toast.error("Please provide a valid mobile number");
                return;
            }

            try {
                setLoading(true);
                let res = await axios.post(`${base_url}/api/auth/generate-otp`, {
                    mobile_number: data.mobile_number
                });
                setOtpShow(true);
                setLoading(false);
                console.log(res);
            } catch (error) {
                setLoading(false);
                console.error(error);
                toast.error("An error occurred. Please try again.");
            }
        }
    }

    return (
        <div style={{ backgroundImage: "url(https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTcDeizwsqRDsaB6UMmO0d2g2372-mO5T64GQ&usqp=CAU)", backgroundSize: 'cover', minHeight: '100vh', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
            <Container style={{
                borderRadius: "200px 0px 0px 0px", marginTop: "40px", backgroundSize: "cover", width: "500px", height: "500px", display: "flex", justifyContent: "center", alignItems: "center"
                , backgroundImage: "url(data:image/jpeg;base64,)",
            }}>
                <form onSubmit={loginFun}>
                    <h1 style={{ fontSize: "100px" }}>LOGIN</h1>
                    <Grid item xs={12}>
                        <TextField
                            fullWidth
                            variant="outlined"
                            label="Enter Your Mobile No."
                            name="mobile_number"
                            value={data.mobile_number}
                            onChange={handleChange}
                        />
                    </Grid>
                    {otpShow &&
                        <Grid item xs={12}>
                            <TextField
                                fullWidth
                                type='text'
                                label="Enter OTP"
                                name="otp"
                                variant="outlined"
                                value={data.otp}
                                onChange={handleChange}
                            />
                        </Grid>
                    }
                    <Grid item xs={12}>
                        <h1 style={{ textAlign: "center", marginTop: "20px", color: "red", fontWeight: "bold", fontSize: "20px" }}>{data.error}</h1>
                    </Grid>
                    <Grid item xs={12}>


                        <Button variant="contained" type='submit' style={{ width: "100%", backgroundColor: "black", color: "slateblue", fontSize: "30px", fontWeight: "bold", marginTop: "40px", borderRadius: "30px" }}>
                            {loading ?
                                <CircularProgress style={{ margin: 'auto' }} /> : "LOGIN"}
                        </Button>

                    </Grid>
                </form>
            </Container>
        </div>
    );
};

export default Login;
